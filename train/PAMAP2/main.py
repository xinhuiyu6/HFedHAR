...
import numpy as np
import torch
from models.model_PAMAP2 import *
from models.Generators_PAMAP2 import *
import copy
...
def main(args):
    ...
    [slmodels.append(copy.deepcopy(base_2).to(device)) for _ in range(3)]
    [slmodels.append(copy.deepcopy(base_3).to(device)) for _ in range(args.clients-args.clients_h-3)]
    ...
    def training(current_round, Correlation):
        ...
        for _ in range(1):
            ...
            for client in range(args.clients):
                ...
                if current_round < args.rounds:  # args.rounds
                    if client < args.clients_h:
                        ...
                        g_f = args.g_f
                        iterations = int(np.ceil(2 * args.l_epochs * (n_steps_unlabel-1) / g_f))
                        iterations_gen = iterations * g_f

                        count, count_dis, D_LOSS, G_LOSS = 0, 0, 0, 0
                        for local_epoch in range(2*args.l_epochs):
                            shuffle_index = torch.randperm(num)
    ...
    for current_round in range(args.rounds):
        ...
        if current_round == 0:
            ...
            mean_f1 = torch.mean(f1_tensor)
            mean_kappa = torch.mean(kappa_tensor)
            print('CURRENT ROUND: {}, ACC: {}, F1:{}, KAPPA:{}'.format(0, mean_acc, mean_f1, mean_kappa))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--d', type=int, default=36)  # features we choose
    parser.add_argument('--window_size', type=int, default=128)
    parser.add_argument('--bs', type=int, default=128)
    parser.add_argument('--l_epochs', type=int, default=10)
    parser.add_argument('--classes', type=int, default=8)
    parser.add_argument('--all_label', default=[0, 1, 2, 3, 10, 11, 12, 13])
    ...
    parser.add_argument('--g_f', type=int, default=15)
    parser.add_argument('--print_f', type=int, default=5)
    parser.add_argument('--save_path', type=str, default='../PAMAP2/logs/')
