#################################
import argparse
from models.model_WISDM import *
from models.Generators_WISDM import *
import copy
...
def main(args):
    ...
    base_2 = ResNet_reduce_filters(input_channel=1, num_classes=args.classes).to(device)
    base_3 = CNN(input_channel=1, num_classes=args.classes).to(device)
    base_4 = ResNet_reduced_layers(input_channel=1, num_classes=args.classes).to(device)
    base_5 = CNN_reduce_layers(input_channel=1, num_classes=args.classes).to(device)
    base_6 = CNN_reduce_filters(input_channel=1, num_classes=args.classes).to(device)

    [slmodels.append(copy.deepcopy(base_1).to(device)) for _ in range(args.clients_h)]
    [slmodels.append(copy.deepcopy(base_2).to(device)) for _ in range(8)]
    [slmodels.append(copy.deepcopy(base_3).to(device)) for _ in range(9)]
    [slmodels.append(copy.deepcopy(base_4).to(device)) for _ in range(9)]
    [slmodels.append(copy.deepcopy(base_5).to(device)) for _ in range(8)]
    [slmodels.append(copy.deepcopy(base_6).to(device)) for _ in range(args.clients-args.clients_h - 34)]
    ...
    trainable_params4 = sum(p.numel() for p in base_4.parameters(recurse=True) if p.requires_grad) / 1000000
    trainable_params5 = sum(p.numel() for p in base_5.parameters(recurse=True) if p.requires_grad) / 1000000
    trainable_params6 = sum(p.numel() for p in base_6.parameters(recurse=True) if p.requires_grad) / 1000000
    print("MODEL PARAMS: {}, {}, {}, {}, {}, {}".format(
        trainable_params1, trainable_params2, trainable_params3, trainable_params4, trainable_params5, trainable_params6))
    ...
    def training(current_round, Correlation):
        ...
        for _ in range(1):
            ...
            for client in range(args.clients):
                ...
                if current_round < args.rounds:
                    if client < args.clients_h:
                        ...
                        g_f = args.g_f
                        iterations = int(np.ceil(5 * args.l_epochs * (n_steps_unlabel-1) / g_f))
                        iterations_gen = iterations * g_f
                        count, count_dis, D_LOSS, G_LOSS = 0, 0, 0, 0
                        for local_epoch in range(5*args.l_epochs):
            ...
            if current_round % args.print_f == 0 or current_round == args.rounds - 1:
                ...
                print('CURRENT ROUND: {}, Average accuracy: {}, F1 score: {}, KAPPA: {}'.format(current_round+1, mean_acc, mean_f1, mean_kappa))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--subject_split_file_path', type=str, default='../../datasplit/WISDM/train-test-split')
    parser.add_argument('--train_subject_path', type=str, default='../../datasplit/WISDM/5/train-subject')
    parser.add_argument('--clients', type=int, default=46)
    parser.add_argument('--clients_h', type=int, default=6)
    ...
    parser.add_argument('--all_label', default=[0, 1, 2, 3, 4])
    ...
    parser.add_argument('--g_f', type=int, default=10)
