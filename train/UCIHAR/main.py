#################################
import argparse
from models.model_UCIHAR import *
from models.Generators_UCIHAR import *
import copy
...
def main(args):
    ...
    base_3 = ResNet_tiny(input_channel=1, num_classes=args.classes).to(device)
    base_4 = CNN_tiny(input_channel=1, num_classes=args.classes).to(device)

    [slmodels.append(copy.deepcopy(base_1).to(device)) for _ in range(args.clients_h)]
    [slmodels.append(copy.deepcopy(base_3).to(device)) for _ in range(10)]
    [slmodels.append(copy.deepcopy(base_2).to(device)) for _ in range(9)]
    [slmodels.append(copy.deepcopy(base_4).to(device)) for _ in range(args.clients-args.clients_h-19)]
    trainable_params1 = sum(p.numel() for p in base_1.parameters(recurse=True) if p.requires_grad) / 1000000
    trainable_params2 = sum(p.numel() for p in base_3.parameters(recurse=True) if p.requires_grad) / 1000000
    trainable_params3 = sum(p.numel() for p in base_2.parameters(recurse=True) if p.requires_grad) / 1000000
    trainable_params4 = sum(p.numel() for p in base_4.parameters(recurse=True) if p.requires_grad) / 1000000
    print("MODEL PARAMS: {}, {}, {}, {}".format(trainable_params1, trainable_params2, trainable_params3, trainable_params4))


    generator_base = Generator2(z_size=args.z_latent_dim, input_feat=1152, fc_layers=args.fc_layers, fc_units=args.fc_units).to(device)
    ...
    discriminator_base = Discriminator2(hidden_dim=1152, output_dim=1).to(device)
    ...
    def training(current_round, Correlation):
            ...
            for client in range(args.clients):
                ...
                for local_epoch in range(args.l_epochs):
                    ...
                    for n in range(n_steps_unlabel):
                        ...
                        loss = cross_e_loss + 1.0 * loss_unsup
                        loss.backward()
        ...
        GEN_data = torch.zeros((0, 1152)).to(device)
        GEN_labels = list()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--augmentation_config_file_path', type=str, default='../configs/augmentation')
    parser.add_argument('--subject_split_file_path', type=str, default='../../datasplit/UCIHAR/train-test-split')
    parser.add_argument('--train_subject_path', type=str, default='../../datasplit/UCIHAR/5/train-subject')
    parser.add_argument('--clients', type=int, default=29)
    parser.add_argument('--clients_h', type=int, default=5)
    parser.add_argument('--d', type=int, default=9)
    parser.add_argument('--window_size', type=int, default=128)
    parser.add_argument('--bs', type=int, default=128)
    parser.add_argument('--l_epochs', type=int, default=20)
    parser.add_argument('--classes', type=int, default=6)
    parser.add_argument('--all_label', default=[0, 1, 2, 3, 4, 5])
